...
void subtract_polynomials(const struct polynomial *poly1, const struct polynomial *poly2, struct polynomial *diff) {
    diff->n = poly1->n + poly2->n;
    diff->t = (struct term *)malloc(diff->n * sizeof(struct term));
    ...
    while (i < poly1->n && j < poly2->n) {
        if (poly1->t[i].exponent > poly2->t[j].exponent) {
            ...
            diff->t[k].coefficient = -poly2->t[j].coefficient;
            diff->t[k++].exponent = poly2->t[j++].exponent;
        } else {
            int coefficient_diff = poly1->t[i].coefficient - poly2->t[j].coefficient;
            if (coefficient_diff != 0) {
                diff->t[k].coefficient = coefficient_diff;
                diff->t[k++].exponent = poly1->t[i].exponent;
            }
            i++;
            j++;
        }
    ...
    // Copy any remaining terms from poly2 with negated coefficients
    while (j < poly2->n) {
        diff->t[k].coefficient = -poly2->t[j].coefficient;
        diff->t[k++].exponent = poly2->t[j++].exponent;
    }
...
int main() {
    ...
    subtract_polynomials(&poly1, &poly2, &diff);
